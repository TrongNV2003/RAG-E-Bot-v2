...
class LlmModel():
    ...
    def text_query(self, query, temperature, chat_history):
        prompt = f"""Chat history:{chat_history}\n<|im_start|>user\nQuestion:{query}<|im_end|>\n<|im_start|>assistant\n"""
        output = self.generate(prompt, temperature)
        return output
    
    
    def retrieve_query(self, query, temperature, threshold, chat_history):
        documents = []
